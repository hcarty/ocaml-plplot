(* OASIS_START *)
(* DO NOT EDIT (digest: 04123a41cf888dcebd472d5f47f80262) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("plplot", ["src"], [])];
     lib_c =
       [("plplot", "src", ["src/camlidlruntime.h"; "src/plplot_core.h"])];
     flags =
       [
          (["oasis_library_plplot_ccopt"; "compile"],
            [
               (OASISExpr.EBool true,
                 S [A "-ccopt"; A "-g"; A "-ccopt"; A "-O2"])
            ]);
          (["oasis_library_plplot_cclib"; "link"],
            [
               (OASISExpr.EBool true, S []);
               (OASISExpr.EBool false, S [A "-cclib"; A "-fake"])
            ]);
          (["oasis_library_plplot_cclib"; "ocamlmklib"; "c"],
            [
               (OASISExpr.EBool true, S []);
               (OASISExpr.EBool false, S [A "-fake"])
            ])
       ];
     includes = []
  }
