...
value camlidl_plplot_core_c_plcolorbar(
	value _v_opt,
	value _v_position,
	value _v_x,
	value _v_y,
	...
	value _v_bb_color,
	value _v_bb_style,
	value _v_low_cap_color,
	value _v_high_cap_color,
	value _v_cont_color,
	value _v_cont_width,
	value _v_ticks,
	value _v_sub_ticks,
	...
	value _v_values)
{
  double *p_colorbar_width; /*out*/
  double *p_colorbar_height; /*out*/
  plplot_colorbar_opt opt; /*in*/
  plplot_position_opt position; /*in*/
  double x; /*in*/
  double y; /*in*/
  double x_length; /*in*/
  double y_length; /*in*/
  ...
  int bb_color; /*in*/
  int bb_style; /*in*/
  double low_cap_color; /*in*/
  double high_cap_color; /*in*/
  int cont_color; /*in*/
  int cont_width; /*in*/
  double ticks; /*in*/
  int sub_ticks; /*in*/
  char const *axis_opts; /*in*/
  char const *label; /*in*/
  int n_values; /*in*/
  double *values; /*in*/
  struct camlidl_ctx_struct _ctxs = { CAMLIDL_TRANSIENT, NULL };
  ...
  camlidl_ml2c_plplot_core_plplot_colorbar_opt(_v_opt, &opt, _ctx);
  camlidl_ml2c_plplot_core_plplot_position_opt(_v_position, &position, _ctx);
  x = Double_val(_v_x);
  y = Double_val(_v_y);
  x_length = Double_val(_v_x_length);
  y_length = Double_val(_v_y_length);
  bg_color = Int_val(_v_bg_color);
  bb_color = Int_val(_v_bb_color);
  bb_style = Int_val(_v_bb_style);
  low_cap_color = Double_val(_v_low_cap_color);
  high_cap_color = Double_val(_v_high_cap_color);
  cont_color = Int_val(_v_cont_color);
  cont_width = Int_val(_v_cont_width);
  ticks = Double_val(_v_ticks);
  sub_ticks = Int_val(_v_sub_ticks);
  axis_opts = String_val(_v_axis_opts);
  label = String_val(_v_label);
  _c1 = Wosize_val(_v_values) / Double_wosize;
  values = camlidl_malloc(_c1 * sizeof(double ), _ctx);
  for (_c2 = 0; _c2 < _c1; _c2++) {
    values[_c2] = Double_field(_v_values, _c2);
  }
  n_values = _c1;
  p_colorbar_width = &_c3;
  p_colorbar_height = &_c4;
  c_plcolorbar(p_colorbar_width, p_colorbar_height, opt, position, x, y, x_length, y_length, bg_color, bb_color, bb_style, low_cap_color, high_cap_color, cont_color, cont_width, ticks, sub_ticks, axis_opts, label, n_values, values);
  Begin_roots_block(_vres, 2)
    _vres[0] = copy_double(*p_colorbar_width);
    _vres[1] = copy_double(*p_colorbar_height);
    _vresult = camlidl_alloc_small(2, 0);
    Field(_vresult, 0) = _vres[0];
    Field(_vresult, 1) = _vres[1];
  End_roots()
...
value camlidl_plplot_core_c_plcolorbar_bytecode(value * argv, int argn)
{
  return camlidl_plplot_core_c_plcolorbar(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], argv[10], argv[11], argv[12], argv[13], argv[14], argv[15], argv[16], argv[17]);
}
